...
export default {
  ...
  methods: {
    editHandler(index) {
      this.$router.push({ name: 'CollaboratorEdit', params: { index: index }})
    },
    removeHandler(index) {
      this.collaboratorList.splice(index, 1)
      if(localStorage.getItem('collaborators')) {
        localStorage.setItem('collaborators', JSON.stringify(this.collaboratorList))
      }
    }
  },
  created() {
    ...
    for (const collaborator of this.collaboratorList) {
      if (collaborator.photo) {
        const imageRef = storageRef(storage, 'images/' + collaborator.photo)
        promises.push(getDownloadURL(imageRef).then((url) => ({ collaborator, url })))
      } else {
    ...
    Promise.all(promises)
      .then((results) => {
        results.forEach(({ collaborator, url }) => {
          const index = this.collaboratorList.findIndex((c) => c === collaborator)
